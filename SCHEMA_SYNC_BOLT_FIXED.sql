-- ============================================================================
-- SCRIPT DE SYNCHRONISATION DU SCHÉMA AVEC BOLT (VERSION CORRIGÉE)
-- ============================================================================
...
-- Supprimer updated_at de persons (non présent dans Bolt selon l'image)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- Ajouter user_id si manquant (VERSION CORRIGÉE)
DO $$
DECLARE
  first_user_id uuid;
BEGIN
  IF NOT EXISTS (
    ...
    -- Met à jour les valeurs existantes avec le premier user
    IF first_user_id IS NOT NULL THEN
      UPDATE relations SET user_id = first_user_id WHERE user_id IS NULL;
    END IF;

    -- Rend la colonne NOT NULL seulement si on a des données
    IF first_user_id IS NOT NULL THEN
...
-- 2. Exécute dans la console : localStorage.clear(); sessionStorage.clear();
-- 3. Redémarre le serveur dev (Ctrl+C puis npm run dev)
-- 4. Teste l'inscription et la création d'événements
-- ============================================================================
